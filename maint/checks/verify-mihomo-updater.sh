...
printf '%s\n' \
	'{"tag_name":"v1.2.4","published_at":"2026-04-25T00:00:00Z"}' \
	> "$MOCK_ROOT/release-v1.2.4.json"
printf '%s\n' \
...
: > "$OUTPUT_FILE"
MAKEFILE_PATH="$MAKEFILE_DIR/Makefile" \
RELEASE_API_URL="file://${MOCK_ROOT}/release-v1.2.4.json" \
TARBALL_URL_TEMPLATE="file://${MOCK_ROOT}/source.tar.gz" \
COMMIT_API_URL_TEMPLATE="file://${MOCK_ROOT}/commit.json" \
MAX_RETRIES=1 \
RETRY_DELAY=0 \
sh "$REPO_ROOT/maint/scripts/update-mihomo-core.sh" --version 1.2.4 --output "$OUTPUT_FILE"

grep -Fq 'need_update=true' "$OUTPUT_FILE"
grep -Fq 'latest_version=v1.2.4' "$OUTPUT_FILE"
grep -Fq 'pkg_version=1.2.4' "$OUTPUT_FILE"
grep -Fq 'published_at=2026-04-25T00:00:00Z' "$OUTPUT_FILE"
grep -Fq 'PKG_VERSION:=1.2.4' "$MAKEFILE_DIR/Makefile"
grep -Fq 'PKG_SOURCE_VERSION:=v1.2.4' "$MAKEFILE_DIR/Makefile"
grep -Fq 'PKG_BUILD_VERSION:=v1.2.4' "$MAKEFILE_DIR/Makefile"

: > "$OUTPUT_FILE"
MAKEFILE_PATH="$MAKEFILE_DIR/Makefile" \
RELEASE_API_URL="file://${MOCK_ROOT}/missing.json" \
MAX_RETRIES=1 \
RETRY_DELAY=0 \
sh "$REPO_ROOT/maint/scripts/update-mihomo-core.sh" --version missing --output "$OUTPUT_FILE"

grep -Fq 'need_update=false' "$OUTPUT_FILE"
grep -Fq 'skip=true' "$OUTPUT_FILE"
grep -Fq 'failure_reason=unable to fetch release' "$OUTPUT_FILE"
